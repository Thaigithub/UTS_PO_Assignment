...
def steepest_descent(
    ...
    while True:
        ...
        if abs(alpha_k * la.norm(jac(x_k))) < x_eps:
            return OptimizeResult(x_k, f_k, True, "change of x is within tolerence", nit)
        x_k1 = x_k + alpha_k * d_k
        d_k1 = -jac(x_k1)
        if abs(f_k - fun(x_k1)) < f_eps:
            return OptimizeResult(x_k, f_k, True, "change of fun is within tolerence", nit)
        if la.norm(jac(x_k1)) < g_eps:
            return OptimizeResult(x_k, f_k, True, "norm of gradient is within tolerence", nit)
        if nit > maxiter:
